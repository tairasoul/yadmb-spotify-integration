import { PagerResolver } from "yadmb-types/types/addonTypes";
import reg from "../defaultRegex.js";
import playdl, { SpotifyAlbum, SpotifyPlaylist, SpotifyTrack, YouTubeVideo } from "play-dl/dist/index.js";
...
export const pager: PagerResolver = {
    name: "spotify",
    priority: 0,
    async available(url) {
        return reg.test(url);
    ...
    async queuedPager(track, index, cache, invalidation) {
        ...
        if (data) {
            ...
            embed.addField("type", track.type, true);
            embed.addField("songs", track.tracks.length.toString(), true);
            const sp = await playdl.spotify(track.tracks[0].url) as SpotifyPlaylist || SpotifyAlbum;
